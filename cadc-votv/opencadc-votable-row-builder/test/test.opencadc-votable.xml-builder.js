...
var xmlData_1_2 =
    ...
    '      <FIELD name="Polygon" datatype="double" arraysize="*" />\n ' +
...
describe('XPath resolution', function() {
    // VOTABLE 1.3
    var xmlDOM13 = new xmldom.DOMParser().parseFromString(
        xmlData_1_3,
        'text/xml'
    );

    var testSubject13 = new opencadcVOBuilder.XPathEvaluator(
        xmlDOM13,
        'votable',
    ...
    // VOTABLE 1.2
    var xmlDOM_1_2 = new xmldom.DOMParser().parseFromString(
        xmlData_1_2,
        'text/xml'
    );

    it('Check 1.2 resolution.', function() {
        var testSubject12 = new opencadcVOBuilder.XPathEvaluator(
            xmlDOM_1_2,
            'votable',
...
describe('Read in simple VOTable.', function() {
    var xmlDOM = new xmldom.DOMParser().parseFromString(
        xmlData_1_2,
        'text/xml'
    );
    ...
    var _testComplete = function(e, args) {
        it('Check Metadata.', function() {
            assert.equal(args.tableMetaData.getFields().length, 10);
        });
    };
    ...
    var _testRowAdd = function(e, args) {
        it('Check row ' + args.rowData.getID(), function() {
            ...
            assert.ok(typeof row.getCells()[6].getValue() === 'string');
            assert.ok(!isNaN(row.getCells()[7].getValue()));
            assert.equal(row.getCells()[7].getValue(), Number(1));
            assert.ok(!isNaN(row.getCells()[8].getValue()));
            assert.equal(row.getCells()[8].getValue(), Number(3072));
        });
    };

    voTableBuilder.subscribe(
        opencadcVOBuilder.events.onDataLoadComplete,
        _testComplete
    );
