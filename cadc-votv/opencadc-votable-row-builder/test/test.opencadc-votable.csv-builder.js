...
describe("Read in simple CSV VOTable.", function ()
{
  ...
  var testSubject = new opencadcVOTableRowBuilder.RowBuilderFactory().createBuilder(input);

  it("Load data.", function ()
  {
    var eventCounter = 0;
    var pageEventCounter = 0;
    var rows = [];  // Maybe check the data?

    var onRowAddEventHandler = function (event, row)
    {
      eventCounter++;
      rows.push(row);
    };
    ...
    testSubject.subscribe(opencadcVOTableRowBuilder.events.onRowAdd, onRowAddEventHandler);
    testSubject.subscribe(opencadcVOTableRowBuilder.events.onPageAddEnd, onPageAddEventHandler);

    testSubject.build();

    assert.equal(eventCounter, 8);
    assert.equal(pageEventCounter, 3);
  });
});
