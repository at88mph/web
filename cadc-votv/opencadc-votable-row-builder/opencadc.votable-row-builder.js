;(function(xpath, opencadcUtil, opencadcVOTable, undefined) {
  'use strict'
  var readerEvents = {
    onDataLoadComplete: 'opencadc-votv:onDataLoadComplete',
    onRowAdd: 'opencadc-votv:onRowAdd',

    // For batch row adding.
    onPageAddStart: 'opencadc-votv:onPageAddStart',
    onPageAddEnd: 'opencadc-votv:onPageAddEnd'
  }
  ...
  RowBuilder.prototype.events = {}

  RowBuilder.prototype.subscribe = function(name, handler) {
  ...
  RowBuilder.prototype.unsubscribe = function(name, handler) {
  ...
  RowBuilder.prototype.fireEvent = function(name, args) {
    if (this.events.hasOwnProperty(name)) {
      ...
      if (xmlVOTableResourceDOMs.length > 0) {
        ...
        for (
          ...
          for (
            ...
            var fieldID =
              xmlFieldID && xmlFieldID !== '' ? xmlFieldID : xmlFieldName
  ...
  function CSVRowBuilder(input) {
    ...
    this.build = function() {
      if (input.data) {
        ...
        this.download(
          input.useRelativeURL ? input.url.getRelativeURI() : input.url.getURI()
        )
      } else {
