...
(function($, xpath, opencadcUtil, opencadcVOTable, undefined) {
   ...
   */
     ...
     */
        this.setLongest = function(longestValues, cellID, newValue) {
            var stringLength =
                newValue && newValue.length ? newValue.length : -1;
   ...
   * @returns {*|Row}
   */
    RowBuilder.prototype.buildRowData = function(
        tableFields,
        rowID,
        rowData,
        longestValues,
        ...
        for (
            ...
            var cellValue = cellField.containsInterval()
                ? stringValue
                : cellDatatype.sanitize(stringValue);
   ...
   */
     ...
     */
        ...
        this.build = function() {
            ...
            var xmlVOTableResourceDOMs = this._getElements(
                '/VOTABLE/RESOURCE[@type="results"]'
            );
            ...
            if (xmlVOTableResourceDOMs.length > 0) {
                ...
                var resourceInfoDOMs = this._getElements(
                    nextResourcePath + '/INFO'
                );
                ...
                var resourceTableDOMs = this._getElements(
                    nextResourcePath + '/TABLE'
                );
                ...
                for (
                    ...
                    var tableFields = [];
                    var resourceTableFieldDOM = this._getElements(
                        nextTablePath + '/FIELD'
                    );
           ...
           */
                    ...
                    for (
                        ...
                        var fieldID =
                            xmlFieldID && xmlFieldID !== ''
                                ? xmlFieldID
                                : xmlFieldName;

                        longestValues[fieldID] = -1;

                        var fieldDescriptionDOM = this._getElements(
                        ...
                        var arraySize = fieldDOM.getAttribute('arraysize');
                        ...
                        var field = new opencadcVOTable.Field(
                            ...
                            arraySize
                                ? dataTypeFactory.createDataType('char')
                    ...
                    // var tableDataRows = [];
                    var rowDataDOMs = this._getElements(
                        nextTablePath + '/DATA/TABLEDATA/TR'
                    );
                    var rowCount = 0;
                    ...
                    for (
                        ...
                        var rowCellsDOM = this._getElements(
                            nextRowPath + '/TD'
                        );
            ...
            var xmlVOTableDescription = this._getElements(
                '/VOTABLE/DESCRIPTION'
            );
            var voTableDescription =
                xmlVOTableDescription.length > 0
   ...
   */
    function CSVRowBuilder(input) {
        ...
        if (input.url && input.data) {
            throw new Error(
                'Only one of URL or CSV Data is supported, not both.'
            );
        }
            ...
            if (input.data) {
                ...
                this.download(
                    input.useRelativeURL
                        ? input.url.getRelativeURI()
                        : input.url.getURI()
                );
            } else {
                console.warn(
                    'Nothing to do.  Set input.url or input data to build.'
                );
            }
        };
     ...
     */
        ...
        function handleInputError() {
            var message =
                'opencadc-votv: Unable to obtain CSV VOTable from URL (' +
                input.url +
                ').';
            console.log(message);
            ...
            try {
                ...
                try {
                    ...
                    try {
                        ...
                        throw new Error(
                            'Unable to create an HTTP request.  Aborting!'
                        );
                    }
                }
            }
