 ...
 *   manipulation methods. This increases the speed dramatically, but can only
 *   be done safely because there are no event handlers or data associated with
 *   any cell/row DOM nodes.  Cell editors must make sure they implement
 *   .destroy() and do proper cleanup.
 */
...
(function ($)
{
   ...
   * @class SlickGrid
   * @constructor
   * @param {Node}              container   Container node to create the grid
   *   in.
   * @param {Array,Object}      data        An array of objects for
   *   databinding.
  ...
  function SlickGrid(container, data, columns, options)
  {
    ...
    // These two variables work around a bug with inertial scrolling in
    // Webkit/Blink on Mac. See http://crbug.com/312427.
    var rowNodeFromLastMouseWheelEvent;  // this node must not be deleted while
                                         // inertial scrolling
    var zombieRowNodeFromLastMouseWheelEvent;  // node that was hidden instead
                                               // of getting deleted
    var zombieRowCacheFromLastMouseWheelEvent;  // row cache for above node
    var zombieRowPostProcessedFromLastMouseWheelEvent;  // post processing
    ...
    function init()
    {
      ...
      $focusSink =
        $("<div tabIndex='0' hideFocus style='position:fixed;width:0;height:0;top:0;left:0;outline:0;'></div>").appendTo($container);

      $headerScroller =
        $("<div class='slick-header ui-state-default' style='overflow:hidden;position:relative;' />").appendTo($container);
      $headers =
        $("<div class='slick-header-columns' style='left:-1000px' />").appendTo($headerScroller);
      $headers.width(getHeadersWidth());

      $headerRowScroller =
        $("<div class='slick-headerrow ui-state-default' style='overflow:hidden;position:relative;' />").appendTo($container);
      $headerRow =
        $("<div class='slick-headerrow-columns' />").appendTo($headerRowScroller);
      $headerRowSpacer =
      ...
      $topPanelScroller =
        $("<div class='slick-top-panel-scroller ui-state-default' style='overflow:hidden;position:relative;' />").appendTo($container);
      $topPanel =
        $("<div class='slick-top-panel' style='width:10000px' />").appendTo($topPanelScroller);
      ...
      $viewport =
        $("<div class='slick-viewport' style='width:100%;overflow:auto;outline:0;position:relative;;'>").appendTo($container);
      ...
      {
        $footerRowScroller =
          $("<div class='slick-footerrow ui-state-default' style='overflow:hidden;position:relative;' />").appendTo($container);
        $footerRow =
          $("<div class='slick-footerrow-columns' />").appendTo($footerRowScroller);
        $footerRowSpacer =
          $("<div style='display:block;height:1px;position:absolute;top:0;left:0;'></div>")
            .css("width", getCanvasWidth() + scrollbarDimensions.width + "px")
    ...
    function finishInitialization()
    {
      if (!initialized)
      {
        ...
        // header columns and cells may have different padding/border skewing
        // width calculations (box-sizing, hello?) calculate the diff so we can
        ...
        disableSelection($headers); // disable all text selection in header
                                    // (including input and textarea)

        if (!options.enableTextSelectionOnCells)
        {
          // disable text selection in grid cells except in input and textarea
          // elements (this is IE-specific, because selectstart event will only
          // fire in IE)
    ...
    function getCanvasWidth()
    {
      var availableWidth = viewportHasVScroll ?
                           viewportW - scrollbarDimensions.width : viewportW;
      var rowWidth = 0;
      ...
      columnElements.each(function (i, e)
                          {
                            ...
                            $("<div class='slick-resizable-handle' />")
                              ...
                              .on("dragstart", function (e, dd)
                              {
                                ...
                                // lock each column's width option to current
                                // width
                                columnElements.each(function (i, e)
                                                    {
                                                      columns[i].previousWidth =
                                                        $(e).outerWidth();
                                                    });
                                if (options.forceFitColumns)
                                {
                                  ...
                                  for (j = i + 1; j < columnElements.length;
                                       j++)
                                  {
                                    c = columns[j];
                                    if (c.resizable)
                                ...
                                maxPageX = pageX +
                                           Math.min(shrinkLeewayOnRight, stretchLeewayOnLeft);
                                minPageX = pageX -
                                           Math.min(shrinkLeewayOnLeft, stretchLeewayOnRight);
                              })
                              .on("drag", function (e, dd)
                              {
                                var actualMinWidth, d = Math.min(maxPageX, Math.max(minPageX, e.pageX)) -
                                                        pageX, x;
                                if (d < 0)
                                { // shrink column
                                  x = d;
                                  for (j = i; j >= 0; j--)
                                  {
                                    ...
                                    if (c.resizable)
                                    {
                                      actualMinWidth = Math.max(c.minWidth ||
                                                                0, absoluteColumnMinWidth);
                                      if (x &&
                                          c.previousWidth + x < actualMinWidth)
                                      {
    ...
    function measureCellPaddingAndBorder()
    {
      ...
      // jquery prior to version 1.8 handles .width setter/getter as a direct
      // css write/read jquery 1.8 changed .width to read the true inner
      // element width if box-sizing is set to border-box, and introduced a
      // setter for .outerWidth so for equivalent functionality, prior to 1.8
      // use .width, and after use .outerWidth
      var verArray = $.fn.jquery.split('.');
      jQueryNewWidthBehaviour =
        (verArray[0] == 1 && verArray[1] >= 8) || verArray[0] >= 2;

      el =
        $("<div class='ui-state-default slick-header-column' style='visibility:hidden'>-</div>").appendTo($headers);
      ...
      el =
        $("<div class='slick-cell' id='' style='visibility:hidden'>-</div>").appendTo(r);
    ...
    function getColumnCssRules(idx)
    {
      if (!stylesheet)
      {
        ...
        for (var i = 0; i < cssRules.length; i++)
        {
          ...
          {
            columnIdx =
              parseInt(matches[0].substr(2, matches[0].length - 2), 10);
    ...
    function autosizeColumns()
    {
      ...
      for (i = 0; i < columns.length; i++)
      {
        ...
        {
          shrinkLeeway +=
            c.width - Math.max(c.minWidth, absoluteColumnMinWidth);
        }
      }
      ...
      while (total > availWidth && shrinkLeeway)
      {
        ...
        for (i = 0; i < columns.length && total > availWidth; i++)
        {
          ...
          var shrinkSize = Math.floor(shrinkProportion *
                                      (width - absMinWidth)) || 1;
        ...
        for (i = 0; i < columns.length && total < availWidth; i++)
        {
          ...
          else
          {
            growSize = Math.min(Math.floor(growProportion * currentWidth) -
                                currentWidth, (c.maxWidth - currentWidth) ||
                       1000000) || 1;
          }
          total += growSize;
    ...
    function applyColumnHeaderWidths()
    {
      ...
      var h;
      for (var i = 0, headers = $headers.children(), ii = headers.length;
           i < ii; i++)
      {
        h = $(headers[i]);
    ...
    function scrollTo(y)
    {
      ...
      {
        vScrollDir =
          (prevScrollTop + oldOffset < newScrollTop + offset) ? 1 : -1;
    ...
    function appendCellHtml(stringArray, row, cell, colspan, item)
    {
      ...
      var m = columns[cell];
      var cellCss = "slick-cell l" + cell + " r" +
    ...
    function removeRowFromCache(row)
    {
      ...
      if (rowNodeFromLastMouseWheelEvent === cacheEntry.rowNode)
      {
        ...
        // ignore post processing cleanup in this case - it will be dealt with
        // later
      }
      else
      {
    ...
    function getViewportHeight()
    {
      return parseFloat($.css($container[0], "height", true)) -
             ...
             (options.showTopPanel ?
              options.topPanelHeight + getVBoxDelta($topPanelScroller) : 0) -
             (options.showHeaderRow ?
              options.headerRowHeight + getVBoxDelta($headerRowScroller) : 0) -
    ...
    function resizeCanvas()
    {
      ...
      updateRowCount();
      handleScroll();
      // Since the width has changed, force the render() to reevaluate
      // virtually rendered cells.
    ...
    function updateRowCount()
    {
      ...
      var numberOfRows = dataLengthIncludingAddNew +
                         (options.leaveSpaceForNewRows ? numVisibleRows - 1 :
                          0);
      ...
      // this helps avoid redundant calls to .removeRow() when the size of the
      // data decreased by thousands of rows
      var r1 = dataLength - 1;
      for (var i in rowsCache)
      {
        if (i > r1)
    ...
    function renderRows(range)
    {
      ...
      for (var i = range.top, ii = range.bottom; i <= ii; i++)
      {
        ...
        rowsCache[i] = {
          ...
          // Cell nodes (by column idx).  Lazy-populated by
          // ensureCellNodesInRowsCache().
          "cellNodesByColumnIdx": [],

          // Column indices of cell nodes that have been rendered, but not yet
          // indexed in cellNodesByColumnIdx.  These are in the same order as
          // cell nodes added at the end of the row.
          "cellRenderQueue": []
        };
    ...
    function startPostProcessing()
    {
      ...
      h_postrender =
        setTimeout(asyncPostProcessRows, options.asyncPostRenderDelay);
    }
    ...
    function startPostProcessingCleanup()
    {
      ...
      h_postrenderCleanup =
        setTimeout(asyncPostProcessCleanupRows, options.asyncPostRenderCleanupDelay);
    }
    ...
    function render()
    {
      ...
      postProcessToRow =
        Math.min(getDataLengthIncludingAddNew() - 1, visible.bottom);
      startPostProcessing();
    ...
    function handleScroll()
    {
      ...
      if (vScrollDist)
      {
        ...
        else
        {
          ...
          {
            page = Math.min(n - 1, Math.floor(scrollTop * ((th - viewportH) /
                                                           (h - viewportH)) *
                                              (1 / ph)));
          }
          offset = Math.round(page * cj);
    ...
    function asyncPostProcessRows()
    {
      ...
      while (postProcessFromRow <= postProcessToRow)
      {
        ...
        for (var columnIdx in cacheEntry.cellNodesByColumnIdx)
        {
          ...
          var m = columns[columnIdx];
          var processedStatus = postProcessedRows[row][columnIdx]; // C=cleanup
                                                                   // and
                                                                   // re-render,
                                                                   // R=rendered
          if (m.asyncPostRender && processedStatus !== 'R')
          {
            ...
            if (node)
            {
              m.asyncPostRender(node, row, getDataItem(row), m, (processedStatus ===
                                                                 'C'));
            }
    ...
    function addCellCssStyles(key, hash)
    {
      ...
      {
        throw "addCellCssStyles: cell CSS hash with key '" + key +
              "' already exists.";
      }
    ...
    function handleKeyDown(e)
    {
      ...
      if (!handled)
      {
        if (!e.shiftKey && !e.altKey && !e.ctrlKey)
        {
          ...
          if (e.which == keyCode.ESCAPE)
          {
            ...
            {
              return; // no editing mode to cancel, allow bubbling and default
                      // processing (exit without cancelling the event)
            }
      ...
      if (handled)
      {
        ...
        try
        {
          e.originalEvent.keyCode = 0; // prevent default behaviour for special
                                       // keys in IE browsers (F3, F5, etc.)
        }
          // ignore exceptions - setting the original event's keycode throws
          // access denied exception for "Ctrl" (hitting control key only,
    ...
    function handleClick(e)
    {
      if (!currentEditor)
      {
        ...
        // IE9+ seems to default DIVs to tabIndex=0 instead of -1, so check for
        // cell clicks directly.
      ...
      //if ((activeCell != cell.cell || activeRow != cell.row) &&
    ...
    {
      trigger(self.onMouseEnter, {cell: getCellFromEvent(e), grid: self}, e);
    }

    function handleMouseLeave(e)
    {
      trigger(self.onMouseLeave, {cell: getCellFromEvent(e), grid: self}, e);
    }

    function cellExists(row, cell)
    {
      return !(row < 0 || row >= getDataLength() || cell < 0 ||
               cell >= columns.length);
    }
      ...
      if (activeCellNode)
      {
        ...
        if (d)
        {
          ...
          activeCellNode.innerHTML =
            formatter(activeRow, activeCell, getDataItemValueForColumn(d, column), column, d, self);
          invalidatePostProcessingResults(activeRow);
        }
      }

      // if there previously was text selected on a page (such as selected text
      // in the edit cell just removed), IE can't set focus to anything else
      // correctly
    ...
    function absBox(elem)
    {
      ...
      while ((elem = elem.parentNode) != document.body)
      {
        ...
        {
          box.visible = box.bottom > elem.scrollTop &&
        ...
        {
          box.visible = box.right > elem.scrollLeft &&
    ...
    function setActiveCell(row, cell)
    {
      ...
      if (row > getDataLength() || row < 0 || cell >= columns.length ||
          cell < 0)
      {
        return;
      }
